...
# ----------------------------
# ✅ DEBUG endpoint: send to your number
# ----------------------------
...
async def debug_send(request: Request, body: DebugSend):
    ...
    if not (channel_id and chat_id and chat_type):
        raise HTTPException(status_code=400, detail="Set TEST_CHANNEL_ID/TEST_CHAT_ID or pass in body")
...
async def process_payload(payload: Dict[str, Any]):
    ...
    try:
        ...
        for msg in messages:
            ...
            # ✅ бот отвечает только inbound
            if direction != "inbound":
                continue
            if not (channel_id and chat_id and chat_type):
                continue

            # ✅ тестовый режим: отвечает только твоему номеру
            ...
            if case.get("caseType") == "lost_and_found":
                if settings.LOST_FOUND_TARGET and wazzup:
                    await wazzup.send_text(
                        channel_id=channel_id,
                        chat_type=settings.LOST_FOUND_TARGET.get("chatType", "whatsapp"),
            ...
            if case.get("caseType") == "complaint":
                ...
                region = resolve_region(ex.get("train"))
