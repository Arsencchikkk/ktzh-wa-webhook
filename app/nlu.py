    ...
    if not train:
        if not re.search(r"\bвагон\s*\d{1,2}\b|\b\d{1,2}\s*вагон\b", t):
            ...
            if m:
                ...
                # не считаем "10" поездом
                if not re.fullmatch(r"\d{1,2}", cand):
                    train = cand

    # 3) тц10 / TC10 / ца80 (буквы + цифры)
    # ✅ минимум 2 буквы, чтобы не ловить "в 19:00" как поезд "В19"
    # ✅ стоп-слова, чтобы не ловить "на 1 час" как поезд "НА1"
    if not train:
        ...
        if m:
            ...
            stop = {
                # предлоги/частицы
                "на", "до", "от", "из", "за", "по", "не", "ну", "да", "ок",
                # слова вагон/место/купе
                "ваг", "вагон", "мест", "место", "куп", "купе",
                # часто в жалобах
                "час", "мин", "минут", "часа",
            }

            if letters not in stop:
...
class SimpleNLU:
    ...
    LOST_KEYS = ("потерял", "потеряла", "забыл", "забыла", "оставил", "оставила", "пропал",
                "вещ", "сумк", "рюкзак", "чемодан")
    ...
    COMPLAINT_KEYS = (
        ...
        "холодно", "печка", "отоплен","не вежлив","не смывает","мусор","нет бумаги","кондиционер не работает","проблемы с","нет салфеток","отсуствует","отсуствие салфеток" # ✅ чтобы "Холодно" тоже шло как complaint, если надо
    )
